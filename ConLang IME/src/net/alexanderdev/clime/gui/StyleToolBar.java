 ...
 * @since Mar 8, 2016, 2:30:03 AM
 */
public class StyleToolBar extends JToolBar {
	private static final long serialVersionUID = -2073601950739559654L;
	...
	private JPanel colorGrid, styleGrid;
	...
	public StyleToolBar(Editor editor) {
		...
		String fontNames[] = Environment.getAllFontNames();
		...
		sizes = new JSpinner(new SpinnerNumberModel(16, 1, 100, 1));
		sizes.addChangeListener(e -> {
			size = (Integer) sizes.getValue();
			apply();
		});
		add(sizes);
		...
		colorGrid = new JPanel(new GridLayout(c.length, c[0].length));
		...
		for (int y = 0; y < c.length; y++) {
			for (int x = 0; x < c[0].length; x++) {
				...
				colors[y][x].setMinimumSize(d);
				colors[y][x].setPreferredSize(d);
				colors[y][x].setMaximumSize(d);
